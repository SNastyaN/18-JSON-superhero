...
document.addEventListener('DOMContentLoaded', function(event){
    let superheroes = JSON.parse(array);
    console.log(superheroes);

    let superherolist = "";
    for (let superhero of superheroes){
        superherolist += `<div class="super"><h2>${superhero.name}</h2><div>Компания:${superhero.universe}</div><div>Настоящее имя:${superhero.alterego}</div><div>Роль:${superhero.occupation}</div><div>Друзья:${superhero.friends}</div><div>Суперспособности:${superhero.superpowers}</div><div><img src="${superhero.url}"></div><div>Дополнительная информация:${superhero.info}</div></div>`;
    }
    document.getElementById('Herocontainer').innerHTML=superherolist;
